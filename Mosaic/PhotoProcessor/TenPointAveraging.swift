...
class TenPointAveraging: LibraryPreprocessing {
    ...
    func begin(complete: @escaping () -> Void) throws -> Void {
        ...
        PHPhotoLibrary.requestAuthorization { (status) in
            switch status {
            case .authorized: let fetchOptions = PHFetchOptions()
                
            self.processAllPhotos(fetchResult: PHAsset.fetchAssets(with: fetchOptions), complete: complete)
            case .denied, .restricted: print("Library Access Denied!")
            case .notDetermined: print("Library Access Not Determined!")
            }
        }
    }
    ...
    private func getAvgOverRegion(image: CIImage, region: CGRect) -> RGBFloat {
        ...
        let avgFilter = CIFilter(name: "CIAreaAverage", withInputParameters: [kCIInputImageKey: image, kCIInputExtentKey: avgRegion])
        let ctx = CIContext(options: nil)
        let cgImge = ctx.createCGImage(avgFilter!.outputImage!, from: avgFilter!.outputImage!.extent)!
        let data: UnsafePointer<UInt8> = CFDataGetBytePtr(cgImge.dataProvider!.data)
